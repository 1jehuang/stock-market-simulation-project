...
def get_portfolio_history(portfolio, days=30):
    ...
    portfolio_history['value'] = portfolio.cash_balance
    ...
    for symbol, position in portfolio.positions.items():
        ...
        if not stock_data.empty:
            stock_value = stock_data['Close'] * position.shares
            portfolio_history['value'] += stock_value
...
def main():
    ...
    # Portfolio Chart
    portfolio_history = get_portfolio_history(portfolio)
    ...
    if not portfolio.positions:
        ...
        for symbol, position in portfolio.positions.items():
            ...
            if not current_data.empty:
                ...
                # Create clickable card that sets session state
    ...
    # Add time interval buttons for portfolio chart
    st.markdown("---")
    ...
    if not portfolio_history.empty:
        ...
        st.write("No portfolio history available yet.")
